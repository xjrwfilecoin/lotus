...
//TODO QZ
func (m *Sealing) pledgeSector2(ctx context.Context, sectorID uint64, existingPieceSizes []uint64, sizes ...uint64) ([]Piece, error) {
	if len(sizes) == 0 {
		return nil, nil
	}
...
*/
	...
	//TODO QZ
	/*for i, size := range sizes {
		 // TODO QZ ppi, err := m.sb.AddRemotePiece(size, sectorID, existingPieceSizes)
		 ppi, err := m.sb.AddPiece(ctx,size, sectorID, r,existingPieceSizes)
		if err != nil {
			return nil, err
		}
