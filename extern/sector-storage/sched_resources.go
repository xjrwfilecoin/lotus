...
func (a *activeResources) canHandleRequest(needRes Resources, wid WorkerID, caller string, res storiface.WorkerResources, req *workerRequest) bool {
	log.Infof("canHandleRequest start %v %v %v %v %v %v", req.sector, req.taskType, caller, len(res.GPUs), needRes.CanGPU, a.gpuUsed)
	...
	log.Infof("canHandleRequest end %v %v %v %v %v %v %v", req.sector, req.taskType, caller, res, needRes, a)
