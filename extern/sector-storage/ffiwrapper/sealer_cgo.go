...
func (sb *Sealer) WorkerAddPiece(ctx context.Context, sector abi.SectorID, pieceSizes []abi.UnpaddedPieceSize, newPieceSize abi.UnpaddedPieceSize, PiecePath string) (abi.PieceInfo, error) {
	return abi.PieceInfo{}, xerrors.New("WorkerAddPiece")
}
...
func (sb *Sealer) AddPiece(ctx context.Context, sector abi.SectorID, existingPieceSizes []abi.UnpaddedPieceSize, pieceSize abi.UnpaddedPieceSize, file storage.Data, PiecePath string) (abi.PieceInfo, error) {

	parent_path := os.Getenv(ssd_parent)
	log.Infof("AddPiece existingPieceSizes = %v  PiecePath = %v", existingPieceSizes, PiecePath)
	if parent_path != "" && PiecePath == "" && len(existingPieceSizes) == 0 {
