...
func (sm *StorageMinerAPI) batchPartitions(ctx context.Context, partitions []api.Partition, proof abi.RegisteredPoStProof) ([][]api.Partition, error) {
	nv, err := sm.Full.StateNetworkVersion(ctx, types.EmptyTSK)
	if err != nil {
		return nil, xerrors.Errorf("getting network version: %w", err)
	}

	partitionsPerMsg, err := policy.GetMaxPoStPartitions(nv, proof)
	if err != nil {
...
func (sm *StorageMinerAPI) DeadlinePost(ctx context.Context, deadline int, random string) error {
	...
	partitionBatches, err := sm.batchPartitions(ctx, partitions, mi.WindowPoStProofType)
	if err != nil {
		return err
	}
